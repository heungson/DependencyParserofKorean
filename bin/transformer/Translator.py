...
class Translator(nn.Module):
    ...
    def _model_decode(self, trg_seq, enc_output, src_seq, device):
        ...
        return F.softmax(self.model.tgt_word_prj(dec_output), dim=-1)


    def _get_init_state(self, src_seq, src_pos, device):
        beam_size = self.beam_size
        enc_output, *_ = self.model.encoder(src_seq, src_pos)
        dec_output = self._model_decode(self.init_seq, enc_output, src_seq, device)
    ...
    def translate_sentence(self, src_seq, src_pos, device):
        ...
        with torch.no_grad():
            ...
            enc_output, gen_seq, scores = self._get_init_state(src_seq, src_pos, device)
            ...
            ans_idx = 0   # default
            len_map = self.len_map.repeat(1, n_bm).view(n_inst * n_bm, max_seq_len)
