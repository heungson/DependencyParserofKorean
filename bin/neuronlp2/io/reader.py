...
class CoNLLXReader(object):
    def __init__(self, file_path=None, word_alphabet=None, char_alphabet=None, pos_alphabet=None, type_alphabet=None):
    ...
    def getSentence(self, sentence, normalize_digits=True, symbolic_root=False, symbolic_end=False, sent_id=0):
        lines = [x.strip().split("\t") for x in sentence if not x.strip().startswith("#")]
        comments = [x.strip() for x in sentence if x.strip().startswith("#")]
        ...
        return DependencyInstance(Sentence(word_seqs, word_id_seqs, char_seqs, char_id_seqs, sent_id, lines, comments), pos_seqs, pos_id_seqs, heads, types, type_ids)
